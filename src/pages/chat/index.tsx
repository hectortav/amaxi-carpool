  ...
  return (
    <div className="mx-auto w-11/12 rounded-xl bg-white  p-4  sm:w-3/4">
      <div className="mb-2 flex flex-col gap-2 text-center">
        {room.from && (
          <div className="mx-auto flex items-center gap-2 rounded-full bg-main/30 px-2 ">
            <BiTrip className="h-4 w-4" /> {room.from?.name}
          </div>
        )}
        {room.to && (
          <div className="mx-auto flex items-center gap-2 rounded-full bg-main/30 px-2">
            <CiFlag1 className="h-4 w-4" /> {room.to?.name}
          </div>
        )}
        <div className="mx-auto flex items-center  gap-2 rounded-full bg-main/30 px-2">
